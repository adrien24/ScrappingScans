...
export class MangaRepositoryPrisma implements IMangaRepository {
  ...
  async findAll(): Promise<Manga[]> {
    try {
      ...
      return mangas.map((manga: Prisma.MangaGetPayload<{}>) =>
        this.mapToManga(manga),
      );
    } catch (error) {
      logger.error("Error finding all mangas", error);
      throw error;
    }
  }
  ...
  async findBySite(site: SiteSource): Promise<MangaWithScans[]> {
    try {
      ...
      return mangas.map(
        (manga: Prisma.MangaGetPayload<{ include: { scans: true } }>) =>
          this.mapToMangaWithScans(manga),
      );
    } catch (error) {
  ...
  private mapToManga(data: Prisma.MangaGetPayload<{}>): Manga {
    return {
      ...
      genres: Array.isArray(data.genres)
        ? data.genres
        : JSON.parse(String(data.genres || "[]")),
      authors: Array.isArray(data.authors)
        ? data.authors
        : JSON.parse(String(data.authors || "[]")),
    };
  }
   ...
   */
  private mapToMangaWithScans(
    data: Prisma.MangaGetPayload<{ include: { scans: true } }>,
  ): MangaWithScans {
    return {
      ...this.mapToManga(data),
      scans:
        data.scans?.map((scan: Prisma.ScanGetPayload<{}>) => ({
          id: scan.id,
          scanId: scan.scanId,
          chapter: scan.chapter,
          title: scan.title,
          description: scan.description,
          images: Array.isArray(scan.images)
            ? scan.images
            : JSON.parse(String(scan.images || "[]")),
          date: scan.date,
