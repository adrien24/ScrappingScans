...
export class MangaController {
  ...
  async getMangaByTitle(req: Request, res: Response): Promise<void> {
    try {
      ...
      const manga = await mangaService.getMangaByTitle(title as string);
   ...
   */
  async getAllMangas(res: Response): Promise<void> {
    try {
      ...
      logger.error("Error getting all mangas", error);
      res.status(500).json({ error: "Internal server error" });
    }
  }
  ...
  async getMangaScans(req: Request, res: Response): Promise<void> {
    try {
      ...
      const scans = await mangaService.getMangaScansByChapterId(
        manga.id,
        Number(chapterId),
      );
  ...
  /**
   * GET /api/mangas/:title/chapters
   * Récupérer tous les scans d'un manga
   */
  async getMangaChapter(req: Request, res: Response): Promise<void> {
    try {
      const { title, chapterId } = req.params;
  ...
  async createManga(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!title || !linkManga || !site) {
        res
          .status(400)
          .json({ error: "Missing required fields: title, linkManga, site" });
        return;
      }

      const manga = await mangaService.createMangaFromMAL(
        title,
        linkManga,
        site,
      );
