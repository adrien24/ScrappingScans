...
export class MangaService {
   ...
   */
  async getMangaScansByChapterId(
    mangaId: string,
    chapterId: number,
  ): Promise<Scan[]> {
    return scanRepository.findByMangaIdAndChapter(mangaId, chapterId);
  }

  /**
   * Obtenir les chapitres d'un manga
   */
  async getMangaChapters(mangaId: string): Promise<Chapter[] | null> {
    return scanRepository.findByChapter(mangaId);
  }
  ...
  async findNewChapters(
    ...
    const existingChapters = existingScans
      .map((scan) => scan.chapter)
      .filter((ch) => ch !== null);

    const newChapters = scrapedChapters
      .map((ch) => ch.chapter)
      .filter(
        (ch) => ch !== null && !existingChapters.includes(ch),
      ) as number[];
